...
|---|-----------|-----------|-----------|---------------------|-----------------|---------|
| 1 | **Supabase** | Base de datos, auth, storage (todos los datos) | **UE — Frankfurt (eu-central-1)** ✅ | Datos en la UE (sin transferencia) | https://supabase.com/legal/dpa | ☐ |
...
| 8 | **Google** | Login OAuth + Google Calendar | EE.UU./global | **DPF** | https://cloud.google.com/terms/data-processing-addendum | ☐ |

> **Calendly:** Pathway ya **no lo usa** como encargado. Si un coach conecta su
> propio Calendly, ese acuerdo es entre el coach y Calendly (no de Pathway).
...
- **Supabase (#1):** confirmado en **eu-central-1 (Frankfurt, Alemania) = UE** ✅.
  Los datos residen en Europa, sin transferencia internacional del store principal.
  El DPA cubre a sus sub-encargados (AWS).
...
- **Google** → cubre el login OAuth **y la agenda nativa** (Google Calendar API, que reemplazó a Calendly). Aplica el *Cloud Data Processing Addendum*; se acepta en Google Cloud console. Guardá copia de `cloud.google.com/terms/data-processing-addendum`.
...
- [x] **Región UE en Supabase** confirmada: eu-central-1 (Frankfurt) ✅.
