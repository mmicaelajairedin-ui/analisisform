...
// ── Plantilla LINDA (mismo chrome que los emails de onboarding): logo + banner
// verde con la CABRA + bloque acento opcional + cuerpo + CTA + footer con baja.
// Unifica el look de TODOS los emails de coach (etapa + reactivación) con la
...
function brandedEmail({ primer, eyebrow, title, cabra, accent, bodyHtml, reward, ctaText, ctaUrl, coachId, greet }) {
  ...
  // Bloque de recompensa REAL (opcional): imagen del badge/medalla que ya existe
  // en la plataforma + el premio verdadero. `reward.img` es una ruta bajo /assets/
  ...
  // Recompensa: imagen del badge/medalla (fondo transparente, sin recuadro) +
  // texto al lado más grande y en negrita para que resalte.
  const rewardRow = reward ? `<tr><td style="padding:12px 30px 2px;"><table cellpadding="0" cellspacing="0" border="0"><tr>
<td width="104" valign="middle" style="padding:4px 8px 4px 0;"><img src="${HOST}/assets/${reward.img}" width="88" alt="" style="display:block;border:0;"></td>
<td valign="middle" style="padding:4px 0 4px 10px;font-family:Arial,Helvetica,sans-serif;font-size:15px;color:#1F5740;line-height:1.55;font-weight:bold;">${reward.text}</td>
</tr></table></td></tr>` : "";
...
<tr><td style="padding:20px 30px;border-bottom:1px solid #EEF3EE;"><img src="${logo}" width="130" alt="Pathway" style="display:block;border:0;"></td></tr>
<tr><td bgcolor="#1F5740" style="padding:26px 30px 24px;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
<td valign="middle" style="width:118px;"><img src="${goat}" width="108" alt="" style="display:block;border:0;"></td>
...
// Usa la plantilla LINDA (brandedEmail: mismo chrome que onboarding) y el MISMO
// texto que la guía del panel (STEPS.tip*), para no inventar copy ni recrear nada.
// La recompensa (reward) referencia la metodología de Logros que YA está armada
// (puntos/medallas/badges), con la imagen del badge/medalla real — sin prometer
// un unlock que no se otorgue. El botón lleva al panel, donde la guía (cabra)
// sigue el paso. Español neutro (tú), igual que el resto de emails.
function stageEmail(kind, primer, coachId) {
  const S = {
    stage_perfil: {
      ...
      reward: { img: "badges/nivel-1.png", text: "Todo lo que haces suma en Logros Pathway: puntos, medallas y badges. Tu camino ya arrancó." },
    ...
    stage_stripe: {
      ...
      reward: { img: "iconos/medalla-bronce.png", text: "Con el cobro listo ya puedes sumar clientes — y cada cliente activo te acerca a tu primera medalla." },
    ...
    stage_invitar: {
      ...
      reward: { img: "badges/enfocado.png", text: "Sumar tu primer cliente te da +50 puntos al instante y te acerca a tu medalla y al badge Enfocado." },
    ...
    stage_cliente: {
      ...
      reward: { img: "badges/productivo.png", text: "Cuando tu cliente entra y avanza, subes de medalla y te acercas al badge Productivo." },
...
// Email de REFERIDOS — la recompensa es el badge **Comunidad** (ya NO 15 días:
// regalar días a coaches que pagan sale caro; el crédito de días quedó apagado en
// stripe-webhook · REFERRAL_DAYS_CREDIT_ENABLED). El badge Comunidad se otorga solo
// cuando alguien se registra con tu link (panel-v2 · pwCheckAutoBadges). Reusa la
// plantilla linda; reemplaza el HTML viejo de onb_referido (que prometía 15 días).
function referralBadgeEmail(primer, coachId) {
  const refLink = HOST + "/registro.html?ref=popup&referrer=" + encodeURIComponent(coachId);
  return brandedEmail({
    primer, eyebrow: "Programa de referidos", title: "Suma coaches a Pathway", cabra: "salta.gif", accent: "",
    bodyHtml: "¿conoces a un coach que encajaría en Pathway? Comparte tu link. Cuando se registra con él, <strong>desbloqueas el badge Comunidad</strong> en tu colección de Logros.",
    reward: { img: "badges/comunidad.png", text: "El badge Comunidad es tuyo cuando un coach entra con tu link." },
    ctaText: "Recomendar Pathway", ctaUrl: refLink, coachId,
  });
}
...
Deno.serve(async (req) => {
  ...
  if (testEmail) {
    ...
    const out = [];
    for (const s of ONB) {
      const html = s.id === "onb_referido" ? referralBadgeEmail("Micaela", sampleId) : fillOnb(s.html, "Micaela", sampleId);
      const ok = await sendEmail(testEmail, "Micaela", fillOnb(s.subject, "Micaela", sampleId), html);
      out.push({ id: s.id, ok });
    }
    ...
    for (const u of coaches) {
      ...
      try {
        ...
        if (trialKind) {
          ...
          // Referidos usa la versión BADGE (no el HTML viejo de 15 días).
          html = onbStep.id === "onb_referido"
            ? referralBadgeEmail(primer, String(u.id))
            : fillOnb(onbStep.html, primer, String(u.id));
          push = onbStep.push;
          ...
          html = brandedEmail({
            primer, eyebrow: "Te extrañamos", title: "Tus clientes te esperan", cabra: "mira.png", accent: "",
            bodyHtml: "hace unos días que no entras a Pathway, y tus clientes te esperan del otro lado. Con un par de minutos <strong>retomas justo donde lo dejaste</strong>. Y si necesitas ayuda con algo, responde este correo y te doy una mano personalmente. Te esperamos. 🌱",
            ctaText: "Volver a mi panel", ctaUrl: GO_CLIENTES, coachId: String(u.id),
          });
