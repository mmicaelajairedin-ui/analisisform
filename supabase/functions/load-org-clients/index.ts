// ===================================================================
// load-org-clients — carga clientes (candidatos) de una organización
// Usa service role para evitar restricciones RLS en cliente.
// Body: { org_id }
// Resp: { ok, data: candidatos[] } | { error }
// ===================================================================
...
async function isOwnerOrAdminOfOrg(id: string, email: string, org_id: string): Promise<boolean> {
  ...
  try {
    // Check if caller is owner of this org, admin, or coach within this org
    const ors: string[] = [];
    ...
    for (const row of rows) {
      // Admin can see clients from any org
      if (row.rol === "admin") return true;
      // Owner can see clients from their org
      if (row.rol === "owner" && row.org_id === org_id) return true;
      // Coach can see clients from their org
      if (row.rol === "coach" && row.org_id === org_id) return true;
    }
    return false;
  } catch { return false; }
}
...
Deno.serve(async (req: Request) => {
  ...
  // Verify caller is owner/admin of this org or coach within this org
  if (!(await isOwnerOrAdminOfOrg(who.id, who.email, org_id))) {
    return json({ error: "not_authorized" }, 403);
  }

  // Load all clients (candidatos) for this org
  try {
    const query = `${SB_URL}/rest/v1/candidatos?org_id=eq.${encodeURIComponent(org_id)}&select=id,nombre,email,coach_id,estado,created_at,foto_perfil,semana_activa&order=nombre`;
    const r = await fetch(query, { headers: svc });
