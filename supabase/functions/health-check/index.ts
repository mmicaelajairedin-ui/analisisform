...
Deno.serve(async (req: Request) => {
  ...
  const [coaches, cands, errs] = await Promise.all([
    q("usuarios?rol=in.(coach,admin)&select=id,nombre,email,rol,auth_id"),
    q("candidatos?select=id,nombre,email,coach_id,nicho,activo,created_at"),
    q(`client_errors?select=ts,kind,email,page,detail&ts=gte.${since}&order=ts.desc&limit=300`),
  ]);
