...
async function sendReminder(
  ...
  // JULIO 2026: Google Meet links come from citas.meet_link (generated by Google Calendar sync)
  const joinLink = c.meet_link || "";
  // Presencial → mostramos el lugar y NO el botón de videollamada. Online → botón a Meet si existe.
  ...
  const botonModo = esPresencial
    ? ``
    : joinLink
      ? `<a href="${joinLink}" target="_blank" rel="noopener" style="display:inline-block;background:#1B4332;color:#fff;text-decoration:none;padding:12px 22px;border-radius:10px;font-weight:700;font-size:14px;margin:2px 0 12px">${EN ? "Join Google Meet" : "Entrar a Google Meet"}</a>`
      : `<p style="font-size:13px;color:#8A968E;margin:12px 0">${EN ? "Google Meet link will appear in Google Calendar" : "El link de Google Meet aparecerá en tu Google Calendar"}</p>`;
