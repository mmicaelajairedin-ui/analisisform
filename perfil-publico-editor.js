...
(function(){
  ...
  window._renderConfig = function(){
    ...
    // El render original es async — espera a que la card "Tu perfil" exista
    var tries=0;
  ...
  function injectPerfilPublico(bioEl){
    if(document.getElementById('pp-card'))return; // ya inyectado

    var ME=window.ME||{};
    if(!ME.id){return;}

    // Trae los campos actuales del coach
    fetch(SB_URL+'/rest/v1/usuarios?id=eq.'+encodeURIComponent(ME.id)+'&select=slug,titulo_profesional,tagline,mi_enfoque,especialidades,atiende,anios_experiencia,perfil_publico_activo&limit=1',{
      ...
      // Insertar despues de la card "Tu perfil" (la que contiene cfg-bio)
      ...
      if(perfilCard&&perfilCard.parentNode){
        ...
        // Fallback: dejar al final del dynamic-panel
